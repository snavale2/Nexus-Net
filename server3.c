...
void handle_server_pass(int client_socket, char *params[], int num_params)
{
    ...
    else
    {
        ...
        {
            printf("Error: Invalid password for client IP %s.\n", client_ip);
            close(client_socket); // Close connection if password does not match
            return;
        }
...
void shutdown_all_connections()
{
    ...
    // Close all server connections (if different from client connections)
    // Assuming you have a way to track server connections similarly
    for (int i = 0; i < MAX_CONNECTED_SERVERS; i++)
    {
        if (serverConfig[i].client_port > 0)
        {
            close(serverConfig[i].client_port);
            serverConfig[i].client_port = 0; // Mark as closed
            printf("Closed server connection socket %d\n", serverConfig[i].client_port);
        }
    }
}
